...
test.describe('Bot ID Credentials Fix Validation', () => {
  test('should use correct credentials when switching between bots in mock reports', async ({ page }) => {
    ...
    expect(optumDisplayedBotId?.trim()).toBe('st-17220bac-4b2e-5e1f-ab83-f8708ce87a8f');
    ...
    expect(compsychDisplayedBotId?.trim()).toBe('st-90549a67-7f19-5074-afcf-3120db51a26d');
    
    // Ensure it's NOT showing the Optum bot ID
    expect(compsychDisplayedBotId?.trim()).not.toBe('st-17220bac-4b2e-5e1f-ab83-f8708ce87a8f');
  ...
  test('should maintain bot-specific analysis results across sessions', async ({ page }) => {
    ...
    await newPage.goto('/');
    await newPage.fill('input[placeholder="Enter your Bot ID"]', 'st-90549a67-7f19-5074-afcf-3120db51a26d');
    ...
    expect(finalOptumReportBotId?.trim()).toBe('st-17220bac-4b2e-5e1f-ab83-f8708ce87a8f');
