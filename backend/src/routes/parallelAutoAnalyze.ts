...
// Start new parallel analysis
parallelAutoAnalyzeRouter.post('/start', async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    // Get bot credentials from session/middleware
    ...
    console.log(`🚀 [ParallelAutoAnalyzeRoute] Starting parallel analysis for bot ${botId} with ${credentials ? 'real' : 'mock'} credentials`);
    
    const parallelAutoAnalyzeService = ParallelAutoAnalyzeService.create(botId, jwtToken, credentials);
    const result = await parallelAutoAnalyzeService.startAnalysis(config);
    console.log(`🚀 [ParallelAutoAnalyzeRoute] Parallel analysis started: ${result.analysisId}`);
    ...
    const response: ApiResponse<never> = {
      success: false,
      error: sanitizeError(error.message || 'Failed to start parallel analysis')
    };
...
// Get parallel analysis progress
parallelAutoAnalyzeRouter.get('/progress/:analysisId', async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const credentials = clientId && clientSecret ? { clientId, clientSecret } : undefined;
    const parallelAutoAnalyzeService = ParallelAutoAnalyzeService.create(botId, jwtToken, credentials);
    const progress = await parallelAutoAnalyzeService.getProgress(analysisId);
    ...
    console.error(`Failed to get parallel analysis progress for ${req.params.analysisId}:`, error);
    
    const statusCode = error.message.includes('not found') ? 404 : 500;
    const response: ApiResponse<never> = {
      success: false,
      error: sanitizeError(error.message || 'Failed to get parallel analysis progress')
    };
...
parallelAutoAnalyzeRouter.get('/results/:analysisId', async (req: Request, res: Response): Promise<void> => {
  try {
    const { analysisId } = req.params;
    ...
    const credentials = clientId && clientSecret ? { clientId, clientSecret } : undefined;
    const parallelAutoAnalyzeService = ParallelAutoAnalyzeService.create(botId, jwtToken, credentials);
    const results = await parallelAutoAnalyzeService.getResults(analysisId);
    ...
    console.error(`Failed to get parallel analysis results for ${req.params.analysisId}:`, error);
    ...
    const response: ApiResponse<never> = {
      success: false,
      error: sanitizeError(error.message || 'Failed to get parallel analysis results')
    };
...
// Cancel parallel analysis
parallelAutoAnalyzeRouter.delete('/:analysisId', async (req: Request, res: Response): Promise<void> => {
  try {
    const { analysisId } = req.params;
    ...
    const credentials = clientId && clientSecret ? { clientId, clientSecret } : undefined;
    const parallelAutoAnalyzeService = ParallelAutoAnalyzeService.create(botId, jwtToken, credentials);
    const cancelled = await parallelAutoAnalyzeService.cancelAnalysis(analysisId);
    ...
    console.error(`Failed to cancel parallel analysis ${req.params.analysisId}:`, error);
    
    const statusCode = error.message.includes('not found') ? 404 : 500;
    const response: ApiResponse<never> = {
      success: false,
      error: sanitizeError(error.message || 'Failed to cancel parallel analysis')
    };
...
// Export parallel analysis results
parallelAutoAnalyzeRouter.get('/export/:analysisId', async (req: Request, res: Response): Promise<void> => {
  try {
    const { analysisId } = req.params;
    ...
    if (analysisId.startsWith('mock-analysis-')) {
      ...
      const parallelAutoAnalyzeService = ParallelAutoAnalyzeService.create(botId, jwtToken);
      ...
      const config = await parallelAutoAnalyzeService.getConfig(analysisId);
    ...
    console.error(`Failed to export parallel analysis ${req.params.analysisId}:`, error);
    ...
    const response: ApiResponse<never> = {
      success: false,
      error: sanitizeError(error.message || 'Failed to export parallel analysis')
    };
...
export default parallelAutoAnalyzeRouter;